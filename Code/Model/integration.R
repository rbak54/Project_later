...
#    temp_extra[,"temperature"]<-Climate_Time_Function(time = time[1:nrow(temp)],min=parms[["Climate_Variables"]][["range_C"]][1],max=parms[["Climate_Variables"]][["range_C"]][2],time_at_peak =parms[["Climate_Variables"]][["time_at_peak"]] )
#    temp_extra[,"R0"]<-find_R0_function(Climate=temp_extra[c(1:nrow(temp)),"temperature"],parms=parms, Climate_Variables_Temp=parms[["Climate_Variables"]], max_R0_Req=F)
...
#out<-out[-1,]
#Sys.time()-a
#write.csv(out,"../../Results/model_results.csv")
############################################################################################################################
#integration summary
a=Sys.time()
extra_cols<-8
...
for (location_index in 1:nrow(data_wider_means_summ)){
  ...
  for (i in peak_contact_seq){
    ...
    mismatch<-round(mismatch,2)
    temp = ode(    y = start,    time = time,    func = SEIR_model,    parms = parms )
    temp_extra<-matrix(rep(c(location_index,data_wider_means_summ[location_index,"minT"],data_wider_means_summ[location_index,"maxT"],data_wider_means_summ[location_index,"peakT"],mismatch,NA,NA,NA)),nrow=nrow(temp),ncol=extra_cols,byrow=T)
    colnames(temp_extra)<-c("country","lower","upper","peak_week","mismatch","temperature","R0","week")
    temp_extra[,"temperature"]<-Climate_Time_Function(time = time[1:nrow(temp)],min=parms[["Climate_Variables"]][["range_C"]][1],max=parms[["Climate_Variables"]][["range_C"]][2],time_at_peak =parms[["Climate_Variables"]][["time_at_peak"]] )
    temp_extra[,"R0"]<-find_R0_function(Climate=temp_extra[c(1:nrow(temp)),"temperature"],parms=parms, Climate_Variables_Temp=parms[["Climate_Variables"]], max_R0_Req=F)
    ...
    temp<-cbind(temp,temp_extra)
    temp<-as_tibble(temp)
    model_means_temp<- temp %>% group_by(country,week,mismatch) %>% summarise(meanI=mean(I),meanR0=mean(R0),meantemp=mean(temperature),.groups="drop")
...
write.csv(model_means,"../../Results/model_results_summary.csv")
#Sys.time()-a
...
model_means<-model_means[-1,]
Sys.time()-a
write.csv(model_means,"../../Results/model_results_combinations_summary.csv")
...
#    mismatch<-round(mismatch,2)
#    temp = ode(    y = start,    time = time,    func = SEIR_model,    parms = parms )
#    temp_extra<-matrix(rep(c(location_index,data_wider_means_summ[location_index,"minT"],data_wider_means_summ[location_index,"maxT"],data_wider_means_summ[location_index,"peakT"],mismatch,NA,NA)),nrow=nrow(temp),ncol=7,byrow=T)
#    colnames(temp_extra)<-c("country","lower","upper","peak_week","mismatch","temperature","R0")
#    temp_extra[,"temperature"]<-Climate_Time_Function(time = time[1:nrow(temp)],min=parms[["Climate_Variables"]][["range_C"]][1],max=parms[["Climate_Variables"]][["range_C"]][2],time_at_peak =parms[["Climate_Variables"]][["time_at_peak"]] )
#    temp_extra[,"R0"]<-find_R0_function(Climate=temp_extra[c(1:nrow(temp)),"temperature"],parms=parms, Climate_Variables_Temp=parms[["Climate_Variables"]], max_R0_Req=F)
#    png(paste0("../../Results/Plots/model_series/",data_wider_means_summ[location_index,"country"],gsub("\\.","",mismatch),".png"))
...
model_means<-model_means[-1,]
write.csv(model_means,"../../Results/model_results_population_summary.csv")
Sys.time()-a
#this way is quicker for more data
########################################################################################################
...
model_means<-model_means[-1,]
write.csv(model_means,"../../Results/model_results_population_combination_summary.csv")
